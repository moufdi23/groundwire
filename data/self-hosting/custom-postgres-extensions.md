...
FROM postgres:$-bookworm AS builder
ARG PG_MAJOR
ARG EXT_VERSION=v1.7.0
RUN apt-get update && apt-get install -y --no-install-recommends \
      build-essential git ca-certificates postgresql-server-dev-$ \
    && rm -rf /var/lib/apt/lists/*
WORKDIR /src
RUN git clone --depth 1 --branch $ https://github.com/fboulnois/pg_uuidv7 .
RUN make
