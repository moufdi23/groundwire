...
as $$
  ...
  begin
    ...
    if user_role is not null then
      -- Set the claim
      claims := jsonb_set(claims, '', to_jsonb(user_role));
    else
      claims := jsonb_set(claims, '', 'null');
    end if;

    -- Update the 'claims' object in the original event
    event := jsonb_set(event, '', claims);
...
const  = supabase.auth.onAuthStateChange(async (event, session) => 
})
```
