...
```javascript
async function fetchData()  = await supabase.from('your_table').select('*')
...
const fetchWithRetry = fetchRetry(fetch, {
  ...
  retryOn: (attempt, error, response) => {
    ...
    if (shouldRetry(attempt, error, response)) `, response)
      return true
    }
...
async function yourDatabaseFunction()  = await supabase
    .rpc('your_database_function', );
