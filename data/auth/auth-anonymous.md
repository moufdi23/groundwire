...
// ---cut---
const  = await supabase.auth.signInAnonymously()
```
...
```c#
var updateEmail = await supabase.Auth.Update(new UserAttributes );

// verify the user's email by clicking on the email change link
// or entering the 6-digit OTP sent to the email address

// once the user has been verified, update the password
var updatePassword = await supabase.Auth.Update(new UserAttributes );
```
...
// ---cut---
const  = await supabase.auth.linkIdentity()
```
...
```c#
var state = await supabase.Auth.LinkIdentity(Provider.Google, new SignInOptions );
