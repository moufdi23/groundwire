...
const channel = supabase.channel('room:lobby:messages', , // Recommended for production
})
```
...
```python
# Create a channel with a descriptive topic name
channel = supabase.channel('room:lobby:messages', params=})
```
...
```swift
// Listen for messages
await channel.onBroadcast(event: "message_sent") 
